  ...
  def __init__(self, model_path, gpu_id):
    ...
    with self.graph.as_default():
      with tf.variable_scope('prior_based_hand'):
        gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.1, visible_device_list=str(gpu_id))
...
class ModelIK:
  ...
  """
  def __init__(self, input_size, network_fn, model_path, net_depth, net_width, gpu_id):
    """
    Parameters
    ----------
    input_size : int
...
class ModelPipeline:
  ...
  """
  def __init__(self, HAND_MESH_MODEL_PATH, DETECTION_MODEL_PATH, IK_MODEL_PATH, gpu_id):
